...
class BetterNavigator extends Component {
	...
	configureScene(route, routeStack) {
		const {REMIT, EVENT_RECEIVER_LIST, ADD_CONTACT, NEW_GROUP} = this.props.constants.Routes;
		const modalCondition = (
			route.name === REMIT
			|| route.name === EVENT_RECEIVER_LIST
			|| route.name === ADD_CONTACT
			|| route.name === NEW_GROUP
		);
	...
	renderScene(route, navigator) {
		if(__DEV__) { console.log(route.name); }
